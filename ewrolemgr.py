...
async def updateRoles(
	...
	if server_default != None:
		user_data = EwPlayer(id_user=member.id, id_server = server_default)
	else:
	...
	try:
		...
		ewutils.logMsg('error: failed to replace roles for {}:{}'.format(member.name, str(e)))
	...
	#	await member.edit(roles=replacement_roles)
	#except:
	#	ewutils.logMsg('error: failed to replace roles for {}'.format(member.name))
...
async def refresh_user_perms(client, id_server, used_member = None, startup = False):
	...
	user_data = EwPlayer(member=used_member)
	...
	if not startup:
		for poi in ewcfg.poi_list:
			...
			if used_member in channel.overwrites and user_data.poi != poi.id_poi:
				...
				try:
					...
					ewutils.logMsg("Failed to remove permissions for {} in channel {}.".format(used_member.name, channel.name))
				...
				try:
					...
					ewutils.logMsg("Failed to add permissions to {} in channel {}.".format(used_member.name, channel.name))
		...
		# Member has no overwrites -- fix this:
			...
			try:
				...
				ewutils.logMsg("Failed to fix permissions for {}:{}.".format(used_member.name, str(e)))
			#time_now_end = int(time.time())
	...
	# 			user_data = EwPlayer(member=current_member)
	# 
	# 			for poi in ewcfg.poi_list:
	# 
	# 				channel = ewutils.get_channel(server, poi.channel)
